...
def test_defaults(monkeypatch: pytest.MonkeyPatch):
    ...
    assert settings.odds_api_base_url == DEFAULT_ODDS_API_BASE_URL
    ...
    assert settings.odds_api_min_request_interval_seconds == 0.25
...
def test_odds_api_env_overrides(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("WNBA_ENGINE_ODDS_API_BASE_URL", "https://example.test/oddsapi")
    monkeypatch.setenv("WNBA_ENGINE_ODDS_API_KEY", "odds-secret")
    monkeypatch.setenv("WNBA_ENGINE_ODDS_API_MIN_REQUEST_INTERVAL_SECONDS", "0.75")
    settings = load_settings()
    assert settings.odds_api_base_url == "https://example.test/oddsapi"
    assert settings.odds_api_key == "odds-secret"
    assert settings.odds_api_min_request_interval_seconds == 0.75


def test_blank_odds_api_key_is_none(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("WNBA_ENGINE_ODDS_API_KEY", "")
    assert load_settings().odds_api_key is None
